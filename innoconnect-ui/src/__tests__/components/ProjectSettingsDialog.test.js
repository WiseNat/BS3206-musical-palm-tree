...
describe("ProjectSettingsDialog", () => {
    it("opens dialog when handleOpen Button clicked", () => {
        ...
        expect(screen.getByRole("heading", {name: toRegex("update project")})).toBeTruthy();
    });
    ...
    it("loads the default project values", () => {
        ...
        expect(screen.getByRole("textbox", { name: toRegex("project description") })).toHaveValue(project.description);
        expect(screen.getByDisplayValue(toRegex(project.mainCommunicationLanguage))).toBeTruthy();
        expect(screen.getByDisplayValue(toRegex(project.mainTimezone))).toBeTruthy();
        expect(screen.getByDisplayValue(toRegex(project.mainProgrammingLanguage))).toBeTruthy();
        expect(screen.getByRole('textbox', { name: toRegex("project url") })).toHaveValue(project.projectUrl);
    });
    ...
    it("dialog closes when 'Save' Button clicked", () => {
        ...
        expect(screen.queryByRole("heading", { name: toRegex("update project") })).toBeNull();
    })
});
